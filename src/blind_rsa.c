...
static int
_blind(RSA_BLIND_MESSAGE *blind_message, RSA_BLIND_SECRET *secret_, RSA *rsa, BN_CTX *bn_ctx,
...
int
RSA_blind(RSA_BLIND_MESSAGE *blind_message, RSA_BLIND_SECRET *secret, RSA *rsa, const uint8_t *msg,
          size_t msg_len)
{
...
int
RSA_blind_sign(RSA_BLIND_SIGNATURE *blind_sig, RSA *rsa, const RSA_BLIND_MESSAGE *blind_message)
{
    if (_rsa_parameters_check(rsa) != 1) {
        return 0;
    }
...
static int
_verify(const RSA_BLIND_SIGNATURE *blind_sig, const RSA_BLIND_SECRET *secret_, RSA *rsa,
        BN_CTX *bn_ctx, const uint8_t msg_hash[HASH_DIGEST_LENGTH])
{
    BIGNUM *secret  = BN_CTX_get(bn_ctx);
    ...
    const int ret = RSA_verify_PKCS1_PSS_mgf1(rsa, msg_hash, evp_md, evp_md, em, -1);
    OPENSSL_free(em);
...
int
RSA_blind_verify(const RSA_BLIND_SIGNATURE *blind_sig, const RSA_BLIND_SECRET *secret, RSA *rsa,
